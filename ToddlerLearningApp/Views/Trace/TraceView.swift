...
//  ToddlerLearningApp
//
//  Trace Letters and Trace Numbers — which one is the view model's `kind`.
//
...
struct TraceView: View {
    ...
    init(viewModel: TraceViewModel, coordinator: AppCoordinator) {
    ...
    var body: some View {
        ZStack {
                    ...
                    if let item = viewModel.currentItem {
                        if isShort {
                            HStack(alignment: .center, spacing: AppSpacing.element) {
                                canvas(item, edge: edge)

                                VStack(spacing: AppSpacing.element) {
                                    header(item)
                                    ProgressBar(value: viewModel.coverage,
                                                tint: AppColors.paletteColor(item.colorIndex))
                                    controls
                                }
            ...
            if viewModel.isRoundComplete {
                RoundCompleteView(
                    starsEarned: viewModel.itemsCompletedThisRound,
    ...
    private func header(_ item: TraceItem) -> some View {
        ArrowNavBar(
            canGoBack: viewModel.canGoBack,
            canGoForward: viewModel.canGoForward,
            itemNoun: viewModel.kind.itemNoun,
            iconSize: 36,
    ...
    private func canvas(_ item: TraceItem, edge: CGFloat) -> some View {
        let tint = AppColors.paletteColor(item.colorIndex)
        let size = edge
        ...
        return ZStack {
            ...
            // Ghost dot demonstrating the stroke. Hidden as soon as the child
            // is tracing on the path; replayed when they go a few seconds
            // without progress. See `TraceViewModel.playDemo`.
        ...
        .accessibilityLabel("Trace \(item.accessibilityName) with your finger")
        ...
        .accessibilityAction(named: "Hear the \(viewModel.kind.itemNoun)") { viewModel.speakCurrentItem() }
    ...
    private var controls: some View {
        HStack(spacing: AppSpacing.element) {
            ...
            // Round progress (spec F27) — how many of this round's items
            // are done, distinct from the coverage bar above, which tracks
            // progress through the *current* one.
            Text("\(viewModel.itemsCompletedThisRound) of \(viewModel.itemsPerRound)")
                .font(AppFonts.caption)
                .foregroundStyle(AppColors.subtitle)
                .lineLimit(1)
